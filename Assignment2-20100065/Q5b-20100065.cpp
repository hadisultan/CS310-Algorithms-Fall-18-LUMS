...
void printvec2(int mflag){
	if(mflag == 1){
		...
		for(int i=0; i<mst2.size(); i++){
			for(int j=0; j<mst2[i].size(); j++){
				// cout << "From: " << mst2[i][j].from << ", to: " << mst2[i][j].to << ", weight: " << v[i][j].weight << endl ;
				cout << "(" << mst2[i][j].from << "," << mst2[i][j].to << ")";
				ms1edges.push_back(mst2[i][j]);
				// if(v[i][j].from &&
				// v[i][j].to &&
				// v[i][j].weight);
			}
...
int findpath2(int n1, int n2){
	lol.insert(n1);
	for(int k=0; k < mst1[n1].size(); k++){
		if(mst1[n1][k].to == n2){
			return 1;
		}
	}
	for(int k=0; k < mst2[n1].size(); k++){
		if(mst2[n1][k].to == n2){
			return 1;
		}
	}
	for(int k=0; k < mst1[n1].size(); k++){
		if(lol.find(mst1[n1][k].to) == lol.end()){
			if(findpath2(mst1[n1][k].to, n1) == 1){
				return 1;
			}
		}
	}
	for(int k=0; k < mst2[n1].size(); k++){
		if(lol.find(mst2[n1][k].to) == lol.end()){
			if(findpath2(mst2[n1][k].to, n1) == 1){
				return 1;
			}
		}
	}
...
int findpath(int n1, int n2){
	lol.insert(n1);
	for(int k=0; k < mst1[n1].size(); k++){
		if(findpath2(mst1[n1][k].to, n2) == 1 || findpath2(mst2[n1][k].to, n2) == 1){
			return mst1[n1][k].to;
		}
	}
	for(int k=0; k < mst2[n1].size(); k++){
		if(findpath2(mst2[n1][k].to, n2) == 1 || findpath2(mst2[n1][k].to, n2) == 1){
			return mst2[n1][k].to;
		}
	}
	return -1;
...
void dont(){
	cout << "MST does not change. " << endl;
	return;
}

int replaceMSTnode(int a, int b, int c, int w)
{
	for(int i=0; i<mst1[a].size(); i++){
		if(mst1[a][i].to == b){
			mst1[a][i].to = c;
			mst1[a][i].weight = w;
			return 1;
...
int main(){
	...
	printvec();
	int n1, n2, w;
	cout << "Enter the first node of the new path: ";
	cin >> n1;
	cout << "Enter the second node of the new path: ";
	cin >> n2;
	cout << "Enter the weight of the new path: ";
	cin >> w;
	edge temp;
	temp.from = n1;
	temp.to = n2;
	temp.weight = w;
	p.push(temp);
	int pathnode = findpath(n1, n2);
	// cout << "Pathnode is: " << pathnode << endl;
	int oldw = findweight(n1, pathnode);
	if(w < oldw){
		int mflag = replaceMSTnode(n1, pathnode, n2, w);
		cout << "MST2: ";
		printvec2(mflag);
	}
	else dont();
